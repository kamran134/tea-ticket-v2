...
| Тестовые карты | `4169741330151778` 06/25 CVV 119 · `5239151747183468` 11/24 CVV2 292 |

Тестовые карты в доках просрочены (июнь 2025 / ноябрь 2024). Если HPP их отклонит —
запросить свежие у куратора банка. Не блокер для написания кода.
...
5. **Таблицы «Ödəniş Statusları», «Errorlar», «PmoDecline Kodları» на сайте доков пустые** —
   публичного списка статусов и кодов ошибок нет. Отсюда требование A6: неизвестный статус —
   ошибка, а не повод молча смапить в `FAILED`.
...
| `FullyPaid` | `SUCCEEDED` | доки |
| `Declined` | `FAILED` | доки |
| `Canceled` | `CANCELLED` | доки |
| `Expired` | `EXPIRED` | доки |
| `Refunded` | `SUCCEEDED` | доки; возвраты вне скоупа |

**Неизвестный статус — бросать ошибку с полным текстом статуса, не мапить в `FAILED`.**
Публичной таблицы статусов нет (A2.6 п.5), список выше собран из примеров, и статусы
преавторизации гарантированно всплывут. Тихий маппинг в `FAILED` означает отмену
оплаченных билетов — цена ошибки несимметрична. Пусть падает и логирует.
...
2. Полный список значений `order.status`, особенно для `Order_DMS`.
...
5. Свежие тестовые карты — в доках просроченные.
